...
<div id="page-login">
  <div class="login-box">
    ...
    <div class="login-error" id="loginError">Usuario o contraseña incorrectos. Intente nuevamente.</div>
    ...
    <div id="tab-inicio" class="tab-panel active">
      <div class="topbar">
        <div>
          ...
          <div class="page-subtitle">Martes 27 de mayo, 2026 · I Semestre · Calidad del Software</div>
        </div>
      ...
      <div class="grid-2">
        <div class="card">
          ...
          <div class="sched-item"><div class="sched-time">7:00 AM</div><div class="sched-bar" style="background:var(--blue);"></div><div><div class="sched-subject">Calidad del Software — 1SF133</div><div class="sched-prof">5 estudiantes inscritos</div><div class="sched-room">🏫 Aula 3B · Piso 2</div></div></div>
          <div class="sched-item"><div class="sched-time">9:00 AM</div><div class="sched-bar" style="background:var(--green);"></div><div><div class="sched-subject">Ingeniería de Software I — 1SF131</div><div class="sched-prof"> estudiantes inscritos</div><div class="sched-room">🏫 Aula 4A · Piso 2</div></div></div>
    ...
    <div id="tab-calificaciones" class="tab-panel">
      <div class="topbar">
        ...
        <div style="display:flex;gap:10px;align-items:center;">
          ...
          <button class="btn btn-secondary" onclick="alert('Descargando reporte del grupo...')">📥 Exportar</button>
        </div>
      </div>
      <div class="card">
      ...
      <div class="grid-2">
          ...
          <div style="display:flex;flex-direction:column;gap:16px;">
            ...
            <div><label style="font-size:15px;font-weight:700;color:var(--text-mid);display:block;margin-bottom:7px;">Dirigido a</label>
              <select id="avisoGrupo" style="width:100%;padding:12px 14px;border:2px solid var(--border);border-radius:var(--radius-sm);font-family:'Nunito',sans-serif;font-size:16px;color:var(--text);background:var(--bg);">
                <option>Grupo 1SF133 — Calidad del Software</option>
                <option>Grupo 1SF131 — Ingeniería de Software I</option>
                <option>Grupo 2SF241 — Pruebas de Software</option>
                <option>Todos mis grupos</option>
              </select>
            </div>
            <div><label style="font-size:15px;font-weight:700;color:var(--text-mid);display:block;margin-bottom:7px;">Tipo</label>
              <select id="avisoTipo" style="width:100%;padding:12px 14px;border:2px solid var(--border);border-radius:var(--radius-sm);font-family:'Nunito',sans-serif;font-size:16px;color:var(--text);background:var(--bg);">
                ...
                <option>⚠️ Urgente</option>
                <option>📅 Recordatorio de fecha</option>
                <option>✅ Resultado / Nota</option>
              </select>
            </div>
        ...
        <div class="card">
          ...
          <div id="avisosPublicados">
            <div class="ann-item"><div class="ann-title">📅 Entrega Proyecto Delta</div><div class="ann-body">El viernes 30 de mayo es la fecha límite. Formato PDF + URL del repositorio GitHub.</div><div class="ann-date">Hoy · Grupo 1SF133</div></div>
            <div class="ann-item amber"><div class="ann-title">⚠️ Examen Parcial — Jueves 29</div><div class="ann-body">El examen se mantiene en horario regular (7:00 AM). Traer calculadora y cédula.</div><div class="ann-date">Ayer · Grupo 1SF133</div></div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div id="tab-reportes" class="tab-panel">
      ...
      <div class="card">
        ...
        <div class="report-row">
          ...
          <button class="btn btn-primary btn-sm report-btn" onclick="alert('✅ Reporte de calificaciones generado.')">📥 Descargar PDF</button>
        </div>
        <div class="report-row">
          <div class="report-icon">✅</div>
          ...
          <button class="btn btn-primary btn-sm report-btn" onclick="alert('Generando reporte de asistencia de Mayo 2026...')">📥 Descargar PDF</button>
        </div>
        <div class="report-row">
          ...
          <button class="btn btn-primary btn-sm report-btn" onclick="alert('Generando reporte de estudiantes en riesgo...')">📥 Descargar PDF</button>
        </div>
      </div>
    </div>
    ...
    comps.forEach(function(c) {
      ...
      fetch(CTX+'/notas', {method:'POST',
        ...
        .then(d=>{ pendientes--; if (!d.ok) errores++; if (pendientes===0) {
          document.getElementById('saveToast').classList.remove('show');
          alert(errores===0 ? 'Calificaciones guardadas correctamente.' : 'Algunas notas no se pudieron guardar.');
...
function sendProfMsg() {
  ...
  if (!to || !body) { alert('Complete el destinatario y el mensaje.'); return; }
  ...
  fetch(CTX+'/mensajes', {method:'POST', headers:{'Content-Type':'application/x-www-form-urlencoded'}, body:params})
    ...
    .then(d=>{
      if(d.ok){
        ...
        alert('Error: ' + (d.error||'No se pudo enviar. Verifica el nombre del destinatario.'));
      }
    }).catch(()=>alert('Error de conexion al enviar el mensaje.'));
}

function publicarAviso() {
  const title = document.getElementById('avisoTitle').value.trim();
  const body  = document.getElementById('avisoBody').value.trim();
  const grupo = document.getElementById('avisoGrupo').value;
  const tipo  = document.getElementById('avisoTipo').value;
  if (!title || !body) { alert('Complete el título y el contenido del aviso.'); return; }
  ...
  div.innerHTML = `<div class="ann-title">${tipo.split(' ')[0]} ${title}</div><div class="ann-body">${body}</div><div class="ann-date">Hoy, ${today} · ${grupo.split('—')[0].trim()}</div>`;
  container.prepend(div);
  document.getElementById('avisoTitle').value = '';
  document.getElementById('avisoBody').value  = '';
  alert('✅ Aviso publicado para: ' + grupo);
}
