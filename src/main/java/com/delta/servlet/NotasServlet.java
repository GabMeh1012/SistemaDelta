...
public class NotasServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
        ...
        String sql = "SELECT e.id, CONCAT(e.nombre,' ',e.apellido) AS estudiante, "
                   ...
                   + "MAX(CASE WHEN n.componente='proyecto'     THEN n.nota END) AS proy, "
                   + "MAX(CASE WHEN n.componente='examen_final' THEN n.nota END) AS ef, "
                   + "(SELECT COUNT(*) FROM notas_historial h WHERE h.inscripcion_id=i.id AND h.componente='parcial1')     AS mod_p1, "
                   + "(SELECT COUNT(*) FROM notas_historial h WHERE h.inscripcion_id=i.id AND h.componente='parcial2')     AS mod_p2, "
                   + "(SELECT COUNT(*) FROM notas_historial h WHERE h.inscripcion_id=i.id AND h.componente='proyecto')     AS mod_proy, "
                   + "(SELECT COUNT(*) FROM notas_historial h WHERE h.inscripcion_id=i.id AND h.componente='examen_final') AS mod_ef "
                   + "FROM inscripciones i "
                   + "JOIN estudiantes e ON e.id = i.estudiante_id "
                   + "LEFT JOIN notas n ON n.inscripcion_id = i.id "
